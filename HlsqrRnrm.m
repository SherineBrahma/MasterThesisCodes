...
B = [];
for a = 1:1:AnglExpNo
    A{InsNo,a} = StatReconsImgCell{InsNo,1}{a,2*AngoNo}.Rnrm;
    IntB = size(StatReconsImgCell{InsNo,1}{a,2*AngoNo}.Rnrm,1);
    B = [B ; IntB];
    %plot(StatReconsImgCell{InsNo,1}{a,2*AngoNo}.Rnrm,'linestyle',LStyle{a},'LineWidth',2)
    %xlim([0 10])
    %hold on
end
C = [C B];
